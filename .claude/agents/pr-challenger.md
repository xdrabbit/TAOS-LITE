...
## What you may do
Read files. Run read-only git and gh commands: `gh pr list/view/checks/diff`, `git log`, `git diff`, `git branch -r --no-merged`, `git merge-base`, `git show`, `grep`. That is all.
...
4. **Orphan work:** `git branch -r --no-merged origin/main` — every remote branch with commits not on main. For each with no open PR, list `git log --oneline origin/main..origin/<branch> | head -5` and flag it as ORPHAN. Unmerged work with no PR is how things get lost.
